...
test_acc2, train_loss2 = read_plot(dataset='mini_imagenet', model='Resnet_DDplus', layers=56, widths=4, Test='accuracy')

fig = plt.figure(figsize=(6, 4))
ax = fig.add_subplot(111)

ax.plot(train_loss2, label='ResNet-DDplus-74-4')
ax.plot(train_loss1, label='ResNet-74-4')
plt.legend(loc='upper right', frameon=True)
# plt.legend([p1, p2], ["ResNet-CDD-74-4", "ResNet-74-4"], loc='upper left')
plt.ylabel("Train Loss", {'weight': 'normal', 'size': 12})  # 设置y坐标
plt.xlabel("epoch", {'weight': 'normal', 'size': 12})  # 设置y坐标
plt.grid(axis="y", ls=":", lw=1, color="gray", alpha=.4)  # 设置网格线
plt.savefig('trainloss.jpeg', dpi=300)
plt.show()
