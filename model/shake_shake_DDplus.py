...
class ShakeResNet(nn.Module):
    ...
    def __init__(self, depth, base_width, num_classes=10, dataset='cifar10', split_factor=1, nc=3, nd=1):
        ...
        self.conv_1 = nn.Conv2d(nc, in_chs[0], 3, padding=1)
        self.bn_1 = nn.BatchNorm2d(in_chs[0])

        self.stage_1 = self._make_layer(n_units, in_chs[0], in_chs[1], stride=nd)
        self.DD1 = DDplus_bottleneck(in_chs[0], in_chs[1], depth=1, dropRate=0.0, stride=nd, r=16)
        self.stage_2 = self._make_layer(n_units, in_chs[1], in_chs[2], stride=2)
        self.DD2 = DDplus_bottleneck(in_chs[1], in_chs[2], depth=1, dropRate=0.0, stride=2, r=16)
        self.stage_3 = self._make_layer(n_units, in_chs[2], in_chs[3], stride=2)
        self.DD3 = DDplus_bottleneck(in_chs[2], in_chs[3], depth=1, dropRate=0.0, stride=2, r=16)
...
def Shake_shake_DDplus(width, num_classes, nc=3, nd=1):
    if width==32:
        return shake_resnet26_2x32d(num_classes=num_classes, nc=nc, nd=nd)
    if width==64:
        return shake_resnet26_2x64d(num_classes=num_classes, nc=nc, nd=nd)
    if width==96:
        return shake_resnet26_2x96d(num_classes=num_classes, nc=nc, nd=nd)