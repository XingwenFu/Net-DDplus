...
def Pyramid(dataset='cifar10', layers=272, width=200, num_classes=10, nc=3, nd=1, blk=1):
    if blk == 1:
        return aa_PyramidNet(dataset=dataset, depth=layers, alpha=width, num_classes=num_classes, bottleneck=True)
    else:
        return aa_PyramidNet(dataset=dataset, depth=layers, alpha=width, num_classes=num_classes, bottleneck=False)
