...
from model.se_resnet import CifarSEResNet, SEBasicBlock, SEBottleneck, CifarSEBasicBlock
...
def Se_net_DDplus(layers=20, num_classes=10, width=1, nc=3, nd=1, pretrained=False, path=''):
    if layers == 18 or layers == 34:
        if layers==18:
            block_num = [2, 2, 2, 2]
        if layers==34:
            block_num = [3, 4, 6, 3]
        Feature_Net = ResNet(SEBasicBlock, block_num, num_classes=num_classes, nc=nc, nd=nd, width=width)
    if layers == 50 or layers == 101 or layers == 152:
        ...
        if layers==152:
            block_num = [3, 8, 36, 3]
        Feature_Net = ResNet(SEBottleneck, block_num, num_classes=num_classes, nc=nc, nd=nd, width=width)
    if (layers-2)%6 == 0:
        b_n= (layers-2)//6
        Feature_Net = CifarSEResNet(CifarSEBasicBlock, n_size=b_n, num_classes=num_classes, reduction=16, nc=nc, nd=nd, width=width)
    if pretrained:
        model_path = path
