...
class CDDNet(nn.Module):
    def __init__(self, in_planes, out_planes, depth=1, dropRate=0.5, stride=1):
        super(CDDNet, self).__init__()
        self.depth = depth
        self.drop = nn.Dropout(dropRate)
        self.norm = nn.BatchNorm2d(out_planes)
        self.CDD = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                             padding=1, bias=True)

    def forward(self, out_CDD, out_B):
        for i in range(self.depth):
            out_CDD = self.CDD(out_CDD)
            out_CDD = out_CDD*out_B
            out_CDD = self.drop(out_CDD)
            out_CDD = self.norm(out_CDD)
        return out_CDD
...
def Shake_shake(width, num_classes, nc=3, nd=1):
    if width==32:
